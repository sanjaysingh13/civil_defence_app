...
URL map:
  ...
  /equipment/add/                        → EquipmentCreateView (Admin: add item, auto asset tag)
...
urlpatterns = [
    ...
    # GET/POST /equipment/add/       → Admin-only create (auto unique_id after save)
    path("add/", views.EquipmentCreateView.as_view(), name="equipment-create"),
    ...
    path(
        "<int:pk>/log/add/",
        views.EquipmentMaintenanceLogCreateView.as_view(),
        name="equipment-log-add",
    ),
    ...
    path(
        "inventory/",
        views.EquipmentInventorySummaryView.as_view(),
    ...
    path(
        "unit/<int:unit_pk>/inventory/",
        views.EquipmentInventoryByUnitView.as_view(),
        name="unit-inventory",
    ),
    # GET  /equipment/unit/<unit_pk>/logs/
    #      → all maintenance log entries for one unit's equipment
    path(
        "unit/<int:unit_pk>/logs/",
        views.EquipmentMaintenanceByUnitView.as_view(),
        name="unit-logs",
    ),
